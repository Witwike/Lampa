...
var TV_CACHE_MAX_ITEMS = 500;

var QUALITY_CACHE_KEY = 'interface_mod_quality_cache_v1';
var Q_CACHE_TIME = 24 * 60 * 60 * 1000;
var Q_CACHE_MAX_ITEMS = 1000;
...
function setTvCache(cache) {
    pruneCache(cache, TV_CACHE_MAX_ITEMS);
    Lampa.Storage.set(TV_CACHE_KEY, cache);
}

function getQualityCache() {
    return Lampa.Storage.get(QUALITY_CACHE_KEY) || {};
}

function setQualityCache(cache) {
    pruneCache(cache, Q_CACHE_MAX_ITEMS);
    Lampa.Storage.set(QUALITY_CACHE_KEY, cache);
}
...
function startMainBadgesObserver() {
    ...
    domObserver = new MutationObserver(function (mutations) {
        ...
        // Handle vote colors, status, age rating in same observer
...
function colorizeSeriesStatus() {
    ...
    function applyStatusColor(el) {
        ...
        if (statusText.indexOf('Заверш') >= 0 || statusText.indexOf('Ended') >= 0) {
            bg = 'rgba(46,204,113,0.8)'; tc = 'white';
        } else if (statusText.indexOf('Отмен') >= 0 || statusText.indexOf('Canceled') >= 0) {
            bg = 'rgba(231,76,60,0.8)'; tc = 'white';
        } else if (statusText.indexOf('Онгоинг') >= 0 || statusText.indexOf('Выход') >= 0 || statusText.indexOf('В процессе') >= 0 || statusText.indexOf('Return') >= 0) {
            bg = 'rgba(243,156,18,0.8)'; tc = 'black';
        } else if (statusText.indexOf('производстве') >= 0 || statusText.indexOf('Production') >= 0) {
            bg = 'rgba(52,152,219,0.8)'; tc = 'white';
        } else if (statusText.indexOf('Заплан') >= 0 || statusText.indexOf('Planned') >= 0) {
            bg = 'rgba(155,89,182,0.8)'; tc = 'white';
        }
    ...
    function applyAgeRatingColor(el) {
        ...
        Object.keys(groups).some(function (k) {
            ...
            for (var i = 0; i < groups[k].length; i++) {
                if (t.indexOf(groups[k][i]) >= 0) { group = k; return true; }
            }
            return false;
        });
...
function loadSettings() {
    ...
    // Синхронизация типа контента (без дублирования загрузки)
...
function startPlugin() {
    // Проверка jQuery
    if (typeof $ === 'undefined') {
        console.error('Interface MOD: jQuery not found');
        return;
    }
