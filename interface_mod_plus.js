(function () {
    ...
    function changeMovieTypeLabels() {
        ...
        function addLabelToCard(card) {
            ...
            // Исправленная логика получения данных
            var metaData = null;
            
            // 1. Пробуем получить данные через jQuery data (уже объект)
            if ($(card).data('card')) {
                metaData = $(card).data('card');
            } 
            // 2. Пробуем альтернативный ключ
            else if ($(card).data('data')) {
                metaData = $(card).data('data');
            }

            var is_tv = false;
            // Проверяем наличие metaData
            if (metaData) {
                if (metaData.media_type === 'tv' || metaData.type === 'tv' || metaData.type === 'serial') {
                    is_tv = true;
                }
            } else {
                // Если данных нет, определяем по классу
                if ($(card).hasClass('card--tv')) is_tv = true;
            }
            ...
            if (metaData) {
                ...
                else if (metaData.release_date && metaData.release_date.length >= 4) year = metaData.release_date.substring(0, 4);
                else if (metaData.first_air_date && metaData.first_air_date.length >= 4) year = metaData.first_air_date.substring(0, 4);
            }
            ...
            if (is_tv && metaData && metaData.number_of_seasons) {
            ...
            if (hasBadges) view.append(badgesContainer);
    ...
    function showAbout() {
        ...
        <div class="about-plugin">
            ...
            <div class="about-plugin__description" style="margin-top: 20px;">
                <div style="color: #fff; font-size: 15px; margin-bottom: 10px;">Версия 2.2.3</div>
                <ul>
                    <li><span>✦</span> Исправлено отображение года и сезонов на карточках</li>
                    <li><span>✦</span> Исправлена логика получения данных (JSON parse error)</li>
    ...
    Lampa.Manifest.plugins = { name: 'Интерфейс мод', version: '2.2.3', description: 'Улучшенный интерфейс для Lampa' };
